    ...
    try {
      ...
      const processOutlineItem = async (item: any, level: number = 1): Promise<OutlineItem> => {
        ...
        if (item.items && item.items.length > 0) {
          processedItem.children = await Promise.all(item.items.map((child: any) => processOutlineItem(child, level + 1)));
        }
        return processedItem;
      };
      const processedOutline = await Promise.all(outline.map((item: any) => processOutlineItem(item)));
...
[
  ...
  const renderOutlineItem = (item: OutlineItem, index: number) => {
    ...
    return <div key={`${item.title}-${index}`} className="space-y-2">
        <div className="flex items-center space-x-2 p-2 rounded hover:bg-gray-50">
          {hasChildren && <Button variant="ghost" size="sm" onClick={() => toggleExpanded(item.title)} className="p-0 h-4 w-4">
              {isExpanded ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
            </Button>}
          {!hasChildren && <div className="w-4" />}
          
          <Checkbox checked={isSelected} onCheckedChange={checked => handleTopicToggle(item.title, !!checked)} />
        ...
        {hasChildren && isExpanded && <div className="ml-6 space-y-1">
